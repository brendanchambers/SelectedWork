function [ C_cycle_summary C_midman_summary C_in_summary C_out_summary C_all_summary ] = clusterF( F )  % F is the weighted directed matrix defining a network

%CLUSTERF account for directed features of the network topology when
...
% WARNING -- weights on F must be >= 0
    ...
    C_cycle_summary = mean(C_cycle);
    C_midman_summary = mean(C_midman);
    C_in_summary = mean(C_in);
    C_out_summary = mean(C_out);
    C_all_summary = mean(C_all);
