...
import matplotlib
env = gym.make('Pendulum-v0')  # observation space: position (x), velocity (x_dot), angle (theta), angular velocity
# cosine theta, sin theta, theta_dot
...
for i_episode in range(N_epochs):
    ...
    proportional_term = observation[0]*K_p # theta
    integral_term = (prior_integral + proportional_term*T_RES)*K_i # appx numerical integral
    derivative_term = observation[1]*K_d # theta-dot
